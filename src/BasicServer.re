...
let listen = (~poll=?, port, handler) => {
  ...
  print_endline("[ Server ] Listening! Open http://localhost:" ++ string_of_int(port));
  print_newline();
  ...
  let pollSock = () => {
    if (canRead(sock)) {
      ...
      let response = try {
        ...
        let res = handler(method, path, header_map);
        switch res {
        | Ok(_, _) => print_endline("[ Server ] 200 - " ++ path)
        | Bad(code, msg) => print_endline("[ Server ] " ++ string_of_int(code) ++ " - " ++ path)
        };
        res
      } {
